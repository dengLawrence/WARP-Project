...
public class ChannelAnalysis {
  ...
  /**
   * Called by constructor. buildChannelAnalysisTable() parses the program schedule (programTable) and WarpDSL
	  ...
	  for (int i = 0; i < programTable.getNumRows(); i++) {
		  for (int j = 0; j < programTable.getNumColumns(); j++) {
			  ...
			  for (int k = 0; k < instructionParametersArray.size(); k++) {
				  /* Set InstructionParameters currInstr to the current InstructionParameters object in the array. */
